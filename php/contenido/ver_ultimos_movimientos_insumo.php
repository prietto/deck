...
<h2>A continuacion se listan los ultimos <?=$num_limit?> movimientos del insumo</h2>
<h2>Insumo: <strong><?=$row_producto['txt_nombre']?></strong></h2>
...
<table width="100%" id="reporte_pago_empleado" border="1" style="border:1px solid grey;" class="tabla_reporte" cellpadding="5" cellspacing="5" >
  ...
  <? 
  	$num_registros = $db->num_registros($cursor_insumo);
    if($num_registros==0){
  ?>
      <tr>
        <td align="center" colspan="5" nowrap="nowrap">El insumo aun no registra movimientos</td>
      </tr>
    
   <? 
 }
  	while($row=$db->sacar_registro($cursor_insumo)){
