...
class seg_permiso_tabla_autonoma{
	...
	FECHA	AUTOR		MODIFICACION
	===========================================================================*/
	function f_get_permisos_modulos_array($cod_perfil_pk){
		global $db;
		
		$query = "	select 	spta.cod_tabla,
							ta.txt_alias as txt_tabla
					from 	seg_permiso_tabla_autonoma spta,
							tabla_autonoma ta
						  
					where 	spta.cod_perfil = $cod_perfil_pk
					and     spta.cod_tabla = ta.cod_tabla
					group by spta.cod_tabla	
					order BY ta.num_orden asc";
		...
		while($row=$db->sacar_registro($cursor)){
			...
			$cod_modulo = $row['cod_tabla'];
			array_push($arrReturn, $cod_modulo);
