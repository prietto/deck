...
class columna_tabla_autonoma{
	...
	===========================================================================*/
  	function f_convertir_tipo_imput($row_info_columna,$ind_registro=NULL){
		...
		// si el indicardor de not null esta en 1 a�ade atribute require
			...
			if($readonly)
				...
				$(function(){
					$('#$txt_nombre_columna').datetimepicker({
						...
						//changeYear:  true, // Muestra comobobox para seleccionar el a�o
			...
			if($readonly){
				...
				"<div id=\"box_input_".$txt_nombre_columna."\">
					<input type				='text' 
							...
							onClick		=\"ver_valor_script_columna(this,event);this.select();\"
							size		='9' 
					 	/> 
						...
						<input 	class		='contenido'  
								...
								title = 'Busqueda avanzada (Ventana emergente)'
						...
						<script>
							$(function(){
								...
								$('#".$txt_nombre_columna."').on('keydown',function(e){

									var keyCode = e.keyCode ? e.keyCode : e.which ? e.which : e.charCode;
									ver_valor_script_columna(this,e);
								...
								$('#".$txt_nombre_columna."').on('click',function(e){
									e.preventDefault();
									e.stopPropagation();
								...
								$('#".$txt_nombre_columna."').on('blur',function(e){
									e.preventDefault();
									e.stopPropagation();
									e.stopImmediatePropagation();
		...
		//=== Evalua datos tipo LISTBOX >>>
			...
			$row_imput['input']	= 	$div_abre.$div_bloquea.'
				<select '.$class.' '.$required.' '.$on_change.' '.$style.' '.$multiple.' 
			...
			if($ind_multiple == true){
				...
				// estilo para no mostrar el select sin el widget cargado ya que afecta el dise�o de la pagina
	...
	function f_consultar_reportes(
		...
		for($i=0; $i<$num_registros; $i++){
			...
			//=== Evalua datos tipo VARCHAR CON O SIN NUMEROS >>>
				...
				//=== Evita con caracteres especiales como �, espacios, etc >>>
...
//		$txt_script_consulta	= 	$reporte_tabla->f_get_consulta( $arr_post['cod_reporte_tabla'], $cod_tabla);
	...
	//						$value	= "<a href='javascript:ver_menu_registro($value)' class='link_display' >$value</a>";
							...
							//=== A�ade radio buton para manipular el registro >>>
			...
			$tabla = "
					...
					<script>
						$(function(){
							...
							// a�ade clase para indicar que es un clon
							...
							$(window).scroll(function(){
								...
								if(pos_scroll > pos_thead){
									...
									// a�adimos propiedades css al elemento clon
	...
	===========================================================================*/
  	function 	f_dar_formato_imputs_tabla_detalle(
		...
		//=== Si no refrescaron la pantalla consigue informaci�n de la base de datos >>>
