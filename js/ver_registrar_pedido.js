...
ind_facturacion_automatica = false;
ind_facturacion_manual = false;
...
$(function(){
	...
	var html = '<p ><label style="font-size:14px !important;" class="combo_solicitud" for="ind_funcion_facturar">Facturacion automatica: </label><input type="checkbox" value="1"  name="ind_funcion_facturar" id="ind_funcion_facturar" /></p>';
	 html += '<p ><label style="font-size:14px !important;" class="combo_solicitud" for="ind_facturacion_manual">Facturacion Manual: </label><input type="checkbox" value="1"  name="ind_facturacion_manual" id="ind_facturacion_manual" /></p>';
	
	$('#panel_izq_opciones').attr({align:'right',valign:'top'});
	$('#panel_izq_opciones').html(html).promise().done(function(){
		console.log($('body').find('input[type="checkbox"]').length);
		console.log($('input[name="ind_funcion_facturar"]').length);	
		var radio_fact_manual 	= 	$('#ind_facturacion_manual');
		var radio_fact_auto		=	$('#ind_funcion_facturar');



		$(radio_fact_auto).click(function(){			
			if($(this).is(':checked')){			 	
				ind_facturacion_automatica = 1;
				// debe ir a la base de datos 
				$(radio_fact_manual).prop('checked',false);
			}  
		}); // fin click function	


		$(radio_fact_manual).click(function(){
			if($(this).is(':checked')){
				$(radio_fact_auto).prop('checked',false);
				navegar_ajax_return(1098,function(a){
					// al hacer clic sobre manual debe pedir el numero de factura 
					// para bloquearlo	
					//console.log(a);
					modal_deck.open({ // funcion que abre ventana
						...
						// valida que ha presionado enter para generar submit
...
$(function(){
	...
	$('#form1').submit(function(e){
		...
		if($('#ind_funcion_facturar').is(':checked')) {  
			...
			add_input('ind_no_factura','hidden',1);  
...
$(function(){
	...
	$('input[name="val_recibido"]').on('focus',function(){
		if(this.value == 0){
			this.value = '';
		}
	});
	...
	$('input[name="val_recibido"]').on('blur',function(){
		if(this.value == ''){
			this.value = '0';
		}
	});
...
//					alert('La cantidad ingresada es mayor a la existente en bodega');
					...
					$('#'+id_this_obj).val('');
					$('input[name="cantidad[]"]').keyup();		
					$('input[name="val_recibido"]').on('keyup');
...
//	$('#tabla_maestro').delegate('input[name="val_recibido"]','keyup',function() {
	$('input[name="val_recibido"]').on('keyup',function() {
		...
		var val_recibido	= $('input[name="val_recibido"]').val();
		...
		if((value_final == 0 || value_final == '') && (val_recibido != ''  || val_recibido != 0 )){
	...
	var value_txt		= $('#cod_estado_pedido option:selected').text();
...
function addRow(node,id_tabla) {
	...
	for (i=0; i < newdiv.length; i++){
		...
		if(newid == $(elementos_div[i]).attr('id') && $(obj_div).hasClass('cod_producto')){
			$(obj_div).remove();
		}
