...
function addRow(node,id_tabla) {
	...
	for (i=0; i < newSel.length; i++){
		...
		var newName = newSel[i].name.substring(0,newSel[i].name.search(/\d/));
	...
	// captura en variable el tipo de input
	var tipo_input = $(newInpt[0]).prop('type');

	// si el primer input es oculto
	if(tipo_input == 'hidden'){

		//var next_combo = $(newInpt[0]).find("input[type='text'],select,textarea").next();
		// busca el siguiente elemento que cumpla con la condicion
		var next_combo = $(newInpt[0]).next("input[type='text'],select,textarea");		
		
		// hace focus al elemento encontrado
		next_combo.focus();
		
		
	}else{

		// busca un elmento de entrada y selecciona el primero que encuentre
		var next_combo = $(myClone).find("input,select,textarea").first();		
		next_combo.focus();
	}
