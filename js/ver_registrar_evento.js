...
function eliminar_fila(node,id_tabla) {
	...
	var arr_select	= tr.getElementsByTagName('select');
...
/*=====2011/12/29========================================================>>>>
DESCRIPCION: 	Metodo para ajustar el calendario y el combo de localidad al ingresar a la pantalla
===========================================================================*/
/*f_ajustar_localidad();
function f_ajustar_localidad(){
	var t 						= 	document.getElementById("tabla_detalle_3");
	//=== Evaluar cual es la posicion para saber si el tema es general o adicional>>>
	for(pos_row=1;pos_row<t.rows.length; pos_row++){
		if( f_es_impar(pos_row) ) 	ind_localidad_general = 1;
		else						ind_localidad_general = 0;	
		tr 				=	t.rows[pos_row];
		var array_input = 	tr.getElementsByTagName('input');	
		var array_select= 	tr.getElementsByTagName('select');
		//borra la segunda daleccion para el combo de localidad	
		array_select[0].selectedIndex = 1;
		array_select[0].remove(2);//elimina el otro registro
	
		if(ind_localidad_general){
			array_select[0].options[1].text = 'General';
			array_select[0].options[1].value = '1';
	
		}else {
			array_select[0].options[1].text = 'Adicional';
			array_select[0].options[1].value = '2';
			array_input[1].setAttribute('onfocus','');//Bloquea el calendario
		}
...
function addRow(node,id_tabla) {
	...
	//==== Evalua si clona la ultima fila o la anterior para que desbloquee el calendario>>>
	num_con_decimales		= t.rows.length/2;
	num_redondo				= redondear_decimales(num_con_decimales, 0) ;
	if(t.rows.length>=3 && num_redondo!=num_con_decimales){
		ultimo_tr	= t.rows[t.rows.length-2];		
	}
	//== Evalua si el codigo del producto es invalido>>>
	...
	//=== Deja por defecto el estado de la funcion  en Activo
	newSel[1].selectedIndex = 1;


	//==== Evalua si esta en la segunda localidad >>>
	num_con_decimales		= t.rows.length/2;
	num_redondo				= redondear_decimales(num_con_decimales, 0) ;
	if(num_redondo!=num_con_decimales){//esta en un numero impar (esta evaluando la segunda localidad
		newInpt[2].focus();
		//==== Valor por defecto igual al de localidad adicional
		if(t.rows.length-3>0) newInpt[2].value	= t.rows[t.rows.length-3].getElementsByTagName('input')[2].value;
		newInpt[1].setAttribute('onfocus','');//Bloquea el calendario
		newSel[0].selectedIndex = 1;//se ubica en el primer registro del combo de localidades
		newSel[0].options[1].text = 'Adicional';
		newSel[0].options[1].value = '2';
		newSel[0].remove(2);//elimina el otro registro
	}else {
	...
	//=== Deja el registro que dice llave primaria nullo porque es un nuevo registro
	newInpt[0].setAttribute('value','');
...
function addRow(node,id_tabla) {
	...
	//=== Evalua Imputs>>>
	for (i=0; i < newInpt.length; i++){
		if ((newInpt[i].type == 'text' && newInpt[i].name!="fec_funcion[]")|| newInpt[i].type == 'hidden' ) newInpt[i].value = '';
		if (newInpt[i].type == 'checkbox' || newInpt[i].type == 'radio') {
			if (tr.getElementsByTagName('input')[i].checked == true)
			newInpt[i].setAttribute('checked',true);
		}
