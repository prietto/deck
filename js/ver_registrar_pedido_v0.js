...
$(function(){
	...
	$('.tabla_detalle').delegate('input[name="cantidad[]"],input[name="val_precio_unitario[]"]','keyup',function(e) {
		...
		if(key == 187){ // el usuario presiono la tecla +
...
//					alert('La cantidad ingresada es mayor a la existente en bodega');
		...
		var total = 0;
		$('input[name="val_total[]"]').each(function(index,element) {  // ciclo para recorrer todos lo inputs con el nombre definido
...
function f_enter(){
	if(ind_enter_bloqueado==false){
		...
		// captura la forma de pago del pedido
		var cod_forma_pago = $('#cod_forma_pago').val();		

		/*if(cod_forma_pago != 1){
			// LLAMA A LA FUNCION CONFIRM  EJECTUTANDO UN CALLBACK PRIMERO Y DEVOLVIENDO UNA RESPUESTA
			confirm("¿Desea generar factura?", function (a) {
				if(a == "si"){ // la funcion callback devuelve un valor dependiendo del boton seleccionado
					add_input('ind_facturar','hidden',1);
					//navegar(1056);
					return false;
